...
//unsigned char nrf_Send(void) {
	...
	SPI_RW_Reg(WRITE_REG + STATUS_REG, MAX_RT);	//nrf_CLEAR max RT bit
...
unsigned char nrf_Recieve(unsigned char * rx_buf) {
	...
	if(((status & RX_DR))||(!(ffstat & 0x01))) {
		...
		SPI_RW_Reg(WRITE_REG + STATUS_REG, RX_DR);	//nrf_CLEAR RX flag
		return YES_DATA;
	} else {
		return NO_DATA;
	}
 ...
 * RX Mode, nrf_SET RX address, writes RX payload width,
...
void initRX(void) {
	...
	SPI_RW_Reg(WRITE_REG + FEATURE, 0x06);		//nrf_SET features for DPL
 ...
 * Description:
 * This function initializes one nRF24L01 device to
 * TX mode, nrf_SET TX address, nrf_SET RX address for auto.ack,
 * fill TX payload, select RF channel, datarate & TX pwr.
 * PWR_UP is nrf_SET, CRC(2 unsigned chars) is enabled, & PRIM:TX.
 * 
